#!/usr/bin/env node
// Insert Mazda vehicles extracted from the pre-processed Bendix CSV.
//
...
//
// Usage:
//   node scripts/insert-mazda-vehicles.js <clean.csv> [--dry-run]
...
if (!csvArg) {
  console.error('Usage: node scripts/insert-mazda-vehicles.js <clean.csv> [--dry-run]');
  process.exit(1);
}
...
async function main() {
  ...
  // 2. Load existing Mazda vehicles from DB to avoid inserting duplicates
  console.log('Loading existing Mazda vehicles from DB...');
  const existing = await sbSelect('vehicles', 'make=eq.Mazda&select=make,model,series,year_from,year_to,engine_code,trim_code,fuel_type');
  console.log(`Existing Mazda vehicles in DB: ${existing.length}`);
  ...
  console.log(`Done. Inserted ${newVehicles.length} Mazda vehicles.`);

  // 4. Verify
  const after = await sbSelect('vehicles', 'make=eq.Mazda&select=id');
  console.log(`Mazda vehicles in DB now: ${after.length}`);
}
