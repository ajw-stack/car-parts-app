#!/usr/bin/env node
// Insert Volkswagen vehicles from volkswagen_brake_catalogue.csv into the vehicles table.
// Reads the cleaned source CSV (notes column already stripped of #PARTNUM suffix).
// Deduplicates in memory before inserting.
//
// Notes:
//   - TDI, BiTDI, SDI, GTD, R5 TDI, V6 TDI, V8 TDI, V10 TDI → Diesel
//   - All others (TSI, FSI, TFSI, V6, V8, VR6, GTI, 4motion, blank) → ULP
//   - AWD drivetrain → notes = "AWD"
//   - 4motion appears in engine_code but drivetrain column is authoritative
//
// Usage:
//   node scripts/importers/insert-volkswagen-vehicles.js scripts/volkswagen_brake_catalogue.csv [--dry-run]
...
if (!csvArg) {
  console.error('Usage: node scripts/importers/insert-volkswagen-vehicles.js <file.csv> [--dry-run]');
  process.exit(1);
}
...
function deriveFuelType(engineCode) {
  ...
  // TDI variants, BiTDI, SDI, GTD are all diesel
  if (/tdi|bitdi|sdi|gtd/.test(ec)) return 'Diesel';
  return 'ULP';
}
...
async function main() {
  ...
  console.log(DRY_RUN ? '=== DRY RUN ===' : '=== INSERT VOLKSWAGEN VEHICLES ===');
  console.log(`Unique vehicles to insert: ${vehicles.length}`);

  if (DRY_RUN) {
    for (const v of vehicles.slice(0, 20)) {
      console.log(`  ${v.make} ${v.model} ${v.series ?? '-'} ${v.year_from ?? '?'}-${v.year_to ?? 'ON'} ${v.engine_litres ?? '?'}L ${v.engine_config ?? ''} ${v.fuel_type} ${v.notes ?? ''}`);
    }
