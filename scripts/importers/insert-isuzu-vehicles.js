#!/usr/bin/env node
// Insert Isuzu vehicles from isuzu_brake_catalogue.csv into the vehicles table.
...
//
// Usage:
//   node scripts/importers/insert-isuzu-vehicles.js scripts/isuzu_brake_catalogue.csv [--dry-run]
...
if (!csvArg) {
  console.error('Usage: node scripts/importers/insert-isuzu-vehicles.js <file.csv> [--dry-run]');
  process.exit(1);
}
...
function deriveFuelType(engineCode) {
  ...
  if (/electric/.test(ec)) return 'Electric';
  if (/hybrid/.test(ec)) return 'Hybrid';
  // Isuzu diesel codes: DDi, CRDi, DiTD, D; plus common codes TD, dCi
  if (/ddi|crdi|ditd|dci|dtec|i-dtec|dtfi|diesel|\btd\b|\bd\b/.test(ec)) return 'Diesel';
  return 'ULP';
}
...
for (const rawLine of lines) {
  ...
  const engineCode   = get('engine_code').trim();   // descriptor: DDi, CRDi, DiTD, D, TD, etc.
  ...
  const engineConfig = engineCode || null; // store descriptor in engine_config
...
async function main() {
  ...
  console.log(DRY_RUN ? '=== DRY RUN ===' : '=== INSERT ISUZU VEHICLES ===');
