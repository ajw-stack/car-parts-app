#!/usr/bin/env node
// Insert Hyundai vehicles from hyundai_brake_catalogue.csv into the vehicles table.
...
//
// Notes:
//   - AWD drivetrain → notes = "HTRAC" (Hyundai's AWD branding)
//   - Model names with lowercase i (i20, i30, i40, i45) are preserved as-is from CSV
//
// Usage:
//   node scripts/importers/insert-hyundai-vehicles.js scripts/hyundai_brake_catalogue.csv [--dry-run]
...
if (!csvArg) {
  console.error('Usage: node scripts/importers/insert-hyundai-vehicles.js <file.csv> [--dry-run]');
  process.exit(1);
}
...
function deriveFuelType(engineCode) {
  ...
  if (/hybrid/.test(ec)) return 'Hybrid';
  // Hyundai diesel codes: CRDi (Common Rail Direct injection), CRDi E-VGT
  if (/crdi|diesel/.test(ec)) return 'Diesel';
  return 'ULP';
}

// ─── Drivetrain → notes ───────────────────────────────────────────────────────
// AWD → "HTRAC" (Hyundai's proprietary AWD branding)
function drivetrainNotes(dt) {
  if (!dt) return null;
  const u = dt.toUpperCase();
  if (u === 'AWD') return 'HTRAC';
  if (u === '4WD') return '4WD';
  return null;
}
...
async function main() {
  ...
  console.log(DRY_RUN ? '=== DRY RUN ===' : '=== INSERT HYUNDAI VEHICLES ===');
