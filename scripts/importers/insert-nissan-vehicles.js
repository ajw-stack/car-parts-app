#!/usr/bin/env node
// Insert Nissan (and Datsun) vehicles from nissan_brake_catalogue.csv into the vehicles table.
...
//
// Usage:
//   node scripts/importers/insert-nissan-vehicles.js scripts/nissan_brake_catalogue.csv [--dry-run]
...
if (!csvArg) {
  console.error('Usage: node scripts/importers/insert-nissan-vehicles.js <file.csv> [--dry-run]');
  process.exit(1);
}
...
function deriveFuelType(engineCode) {
  ...
  if (/ima|hybrid|e:hev/.test(ec)) return 'Hybrid';
  if (/dci|dtec|i-dtec|dtfi|diesel|\btd\b|\bd\b/.test(ec)) return 'Diesel';
  return 'ULP';
}
...
for (const rawLine of lines) {
  ...
  const engineCode   = get('engine_code').trim();   // descriptor: Turbo, dCi, Electric, etc.
...
async function main() {
  ...
  console.log(DRY_RUN ? '=== DRY RUN ===' : '=== INSERT NISSAN/DATSUN VEHICLES ===');
