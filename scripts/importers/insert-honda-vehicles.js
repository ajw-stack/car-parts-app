#!/usr/bin/env node
// Insert Honda vehicles from honda_brake_catalogue.csv into the vehicles table.
...
//
// Usage:
//   node scripts/importers/insert-honda-vehicles.js scripts/honda_brake_catalogue.csv [--dry-run]
...
if (!csvArg) {
  console.error('Usage: node scripts/importers/insert-honda-vehicles.js <file.csv> [--dry-run]');
  process.exit(1);
}
...
function deriveFuelType(engineCode) {
  ...
  if (/dtec|i-dtec|dtfi|disel|diesel|\bTD\b|\bD\b/.test(ec)) return 'Diesel';
  return 'ULP';
}
...
for (const rawLine of lines) {
  ...
  const engineCode   = get('engine_code').trim();   // descriptor: VTEC, IMA, etc.
...
async function main() {
  ...
  console.log(DRY_RUN ? '=== DRY RUN ===' : '=== INSERT HONDA VEHICLES ===');
