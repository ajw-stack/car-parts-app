#!/usr/bin/env node
// Insert Holden VH Commodore (1981-1984) — 141,018 built (including HDT)
// Usage: node scripts/insert-holden-commodore-vh.js [--dry-run]
// Mild facelift of VB/VC. SS launched March 1982 (253 dual exhaust 115kW).
// MacPherson strut front / 5-link panhard rod rear.

const fs   = require('fs');
const path = require('path');

const DRY_RUN = process.argv.includes('--dry-run');

const envContent = fs.readFileSync(path.join(__dirname, '..', '..', '.env.local'), 'utf8');
const env = {};
for (const line of envContent.split(/\r?\n/)) {
  const m = line.match(/^([^=]+)=(.*)$/);
  if (m) env[m[1].trim()] = m[2].trim();
}
const BASE = env.NEXT_PUBLIC_SUPABASE_URL;
const KEY  = env.SUPABASE_SERVICE_ROLE_KEY;
const hdrs = { apikey: KEY, Authorization: `Bearer ${KEY}`, 'Content-Type': 'application/json' };

async function api(urlPath, options = {}) {
  const res = await fetch(`${BASE}/rest/v1${urlPath}`, { headers: hdrs, ...options });
  const text = await res.text();
  if (!res.ok) throw new Error(`${res.status} ${urlPath}: ${text}`);
  return text ? JSON.parse(text) : null;
}

function splitByTrimCode(arr) {
  return [arr.filter(v => !v.trim_code), arr.filter(v => v.trim_code)];
}

const vehicles = [

  // 1.9L Starfire I4 — 54kW
  {
    make: 'Holden', model: 'Commodore', series: 'VH',
    year_from: 1981, year_to: 1984,
    engine_code: '1900 Starfire', engine_litres: 1.892, engine_config: 'I4',
    engine_kw: 54, fuel_type: 'ULP',
    notes: 'Base engine; Sedan and Wagon; 4-speed MC6 or 5-speed M76 manual, or TriMatic auto',
    specs: {
      engine_description: '1892cc OHV I4 Starfire (8.7:1)',
      torque_nm: 138,
      compression: '8.7:1',
      bore_stroke_mm: '88.9 x 76.2',
      power_rpm: 4400,
      fuel_system: 'GM Strasbourg Varajet twin-barrel downdraft carburettor',
      grades: ['Commodore SL'],
      num_built: 141018,
    },
  },
  // 2.85L 161 I6 — 73kW
  {
    make: 'Holden', model: 'Commodore', series: 'VH',
    year_from: 1981, year_to: 1984,
    engine_code: '161', engine_litres: 2.838, engine_config: 'I6',
    engine_kw: 73, fuel_type: 'ULP',
    notes: 'SL / SL/X; Sedan and Wagon; 5-speed M76 manual or TriMatic auto',
    specs: {
      engine_description: '161ci 2838cc OHV I6 (9.0:1)',
      torque_nm: 187,
      compression: '9.0:1',
      bore_stroke_mm: '88.9 x 76.2',
      power_rpm: 4400,
      fuel_system: 'GM Strasbourg twin-barrel downdraft carburettor',
      grades: ['Commodore SL', 'Commodore SL/X'],
    },
  },
  // 3.3L 202 I6 — 83kW
  {
    make: 'Holden', model: 'Commodore', series: 'VH',
    year_from: 1981, year_to: 1984,
    engine_code: '202', engine_litres: 3.298, engine_config: 'I6',
    engine_kw: 83, fuel_type: 'ULP',
    notes: 'SL / SL/X / SL/E; Sedan and Wagon',
    specs: {
      engine_description: '202ci 3298cc OHV I6 (8.8:1)',
      torque_nm: 231,
      compression: '8.8:1',
      bore_stroke_mm: '92.08 x 82.6',
      power_rpm: 4000,
      fuel_system: 'GM Strasbourg twin-barrel downdraft carburettor',
      grades: ['Commodore SL', 'Commodore SL/X', 'Commodore SL/E'],
    },
  },
  // 4.2L 253 V8 — 100kW single / 115kW dual; SS model uses dual exhaust (March 1982)
  {
    make: 'Holden', model: 'Commodore', series: 'VH',
    year_from: 1981, year_to: 1984,
    engine_code: '253', engine_litres: 4.142, engine_config: 'V8',
    engine_kw: 100, fuel_type: 'ULP',
    notes: 'Optional on SL/SL/X/SL/E; SS uses 115kW dual exhaust tune (March 1982)',
    specs: {
      engine_description: '253ci 4142cc OHV V8 Rochester Quadrajet (9.0:1)',
      torque_nm: 269,
      torque_nm_dual_exhaust: 289,
      compression: '9.0:1',
      bore_stroke_mm: '92.1 x 77.8',
      power_rpm: 4200,
      engine_kw_dual_exhaust: 115,
      fuel_system: 'Rochester Quadrajet 4-barrel downdraft carburettor',
      exhaust: 'Single with crossover and two mufflers; optional/SS dual pipes with three mufflers',
      grades: ['Commodore SL', 'Commodore SL/X', 'Commodore SL/E', 'Commodore SS'],
    },
  },
  // 5.0L 308 V8 — 126kW dual exhaust
  {
    make: 'Holden', model: 'Commodore', series: 'VH',
    year_from: 1981, year_to: 1984,
    engine_code: '308', engine_litres: 5.044, engine_config: 'V8',
    engine_kw: 126, fuel_type: 'ULP',
    notes: 'Optional on SL/E; dual exhaust standard',
    specs: {
      engine_description: '308ci 5044cc OHV V8 Rochester Quadrajet (9.2:1)',
      torque_nm: 361,
      compression: '9.2:1',
      bore_stroke_mm: '101.6 x 77.7',
      power_rpm: 4200,
      fuel_system: 'Rochester Quadrajet 4-barrel downdraft carburettor',
      exhaust: 'Dual pipes with three mufflers',
      grades: ['Commodore SL/E'],
    },
  },

];

async function main() {
  console.log(DRY_RUN ? '=== DRY RUN ===' : '=== INSERT VH COMMODORE ===');
  console.log(`Prepared ${vehicles.length} vehicles`);

  if (DRY_RUN) {
    for (const v of vehicles) {
      console.log(`  Commodore ${v.series} ${v.year_from}-${v.year_to} ${v.engine_code} ${v.engine_kw}kW`);
    }
    return;
  }

  const existing = await api('/vehicles?make=eq.Holden&model=eq.Commodore&series=eq.VH&select=series,year_from,year_to,engine_code,trim_code,fuel_type');
  const existingKeys = new Set(
    existing.map(v => `${v.series ?? ''}|${v.year_from}|${v.year_to ?? ''}|${v.engine_code ?? ''}|${v.trim_code ?? ''}|${v.fuel_type ?? ''}`)
  );

  const toInsert = vehicles.filter(v => {
    const key = `${v.series ?? ''}|${v.year_from}|${v.year_to ?? ''}|${v.engine_code ?? ''}|${v.trim_code ?? ''}|${v.fuel_type ?? ''}`;
    return !existingKeys.has(key);
  });

  console.log(`Already in DB: ${vehicles.length - toInsert.length} | To insert: ${toInsert.length}`);
  if (toInsert.length === 0) { console.log('Nothing to insert.'); return; }

  const r = await api('/vehicles', {
    method: 'POST',
    headers: { ...hdrs, Prefer: 'return=representation' },
    body: JSON.stringify(toInsert),
  });
  if (!Array.isArray(r)) { console.error('Unexpected:', r); process.exit(1); }

  console.log(`Inserted ${r.length} vehicles:`);
  for (const v of r) {
    console.log(`  ${v.id}  Commodore ${v.series} ${v.year_from}-${v.year_to} ${v.engine_code} ${v.engine_kw}kW`);
  }
}

main().catch(err => { console.error(err); process.exit(1); });
