...
async function main() {
  ...
  const existing = await sbSelect('vehicles', 'make=eq.Ford&select=make,model,series,year_from,year_to,engine_code,trim_code,fuel_type');
  ...
  // 4. Verify
  const after = await sbSelect('vehicles', 'make=eq.Ford&select=id');
