#!/usr/bin/env node
// Insert Ford vehicles extracted from the pre-processed Bendix CSV.
//
...
//
// Usage:
//   node scripts/insert-ford-vehicles.js <clean.csv> [--dry-run]
...
if (!csvArg) {
  console.error('Usage: node scripts/insert-ford-vehicles.js <clean.csv> [--dry-run]');
  process.exit(1);
}
...
async function main() {
  ...
  // 2. Load existing Ford vehicles from DB to avoid inserting duplicates
  console.log('Loading existing Ford vehicles from DB...');
  const existing = await sbSelect('vehicles', 'make=eq.Mitsubishi&select=make,model,series,year_from,year_to,engine_code,trim_code,fuel_type');
  console.log(`Existing Ford vehicles in DB: ${existing.length}`);
  ...
  console.log(`Done. Inserted ${newVehicles.length} Ford vehicles.`);

  // 4. Verify
  const after = await sbSelect('vehicles', 'make=eq.Mitsubishi&select=id');
  console.log(`Ford vehicles in DB now: ${after.length}`);
}
